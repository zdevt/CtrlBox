...
std::shared_ptr<OpticEngine> OpticEngineP4A::GetObj ( std::string engineName )
{
  return std::make_shared<OpticEngineP4A> ( engineName );
}

OpticEngineP4A::OpticEngineP4A ( std::string engineName )
  : OpticEngineThin ( engineName )
{
}

OpticEngineP4A::~OpticEngineP4A()
{
}

uint32_t OpticEngineP4A::GetEngineRes ( int& width, int& height )
{
  width = 1920;
  height = 1080;
  return OPTIC_ENGINE_RES_1080P;
}
uint32_t OpticEngineP4A::GetEngineType()
{
  return OPTIC_ENGINE_TYPE_LED;
}

uint32_t OpticEngineP4A::GetEnginePlatform()
{
  return OPTIC_ENGINE_PLATFORM_DDP442X;
}

uint32_t OpticEngineP4A::GetEngine2Dor3D()
{
  return OPTIC_ENGINE_2D;
}

int OpticEngineP4A::WriteLogoAndTestPic()
{
  WriteLogo();
  WriteTestPic();
  return 0;
...
int OpticEngineP4A::GetPowerState ( uint8_t& state )
{
  int istate = 0;
  int temp = 1;
  #if defined(__arm__)
...
int OpticEngineP4A::SetFpgaPixel( )
{
  int w = 0;
  int h = 0;
  GetEngineRes ( w, h );
...
int OpticEngineP4A::SetBcMode ( uint8_t mode )
{
...
int OpticEngineP4A::GetBcMode ( uint8_t& mode )
{
  int imode = 0;
  int temp = 0;
...
int OpticEngineP4A::SetDmdGamma ( uint8_t gammaType )
{
...
int OpticEngineP4A::GetDmdGamma ( uint8_t& gammaType )
{
  int igammaType = 0;
  int temp = 0;
...
int OpticEngineP4A::SetProjMode ( uint8_t mode )
{
...
int OpticEngineP4A::GetProjMode ( uint8_t& mode )
{
  int imode = 0;
  int temp = 0;
...
int OpticEngineP4A::SetColorOffset ( uint16_t r, uint16_t g, uint16_t b )
{
...
int OpticEngineP4A::GetColorOffset ( uint16_t& r, uint16_t& g, uint16_t& b )
{
...
int OpticEngineP4A::SetColorBrightness ( uint16_t r, uint16_t g, uint16_t b )
{
...
int OpticEngineP4A::GetColorBrightness ( uint16_t& r, uint16_t& g, uint16_t& b )
{
...
int OpticEngineP4A::SetColorContrast ( uint16_t r, uint16_t g, uint16_t b )
{
...
int OpticEngineP4A::GetColorContrast ( uint16_t& r, uint16_t& g, uint16_t& b )
{
...
int OpticEngineP4A::SetDutycycle ( uint8_t dc )
{
  int idc = dc;
...
int OpticEngineP4A::GetDutycycle ( uint8_t* dc )
{
  int idc;
  int temp = 0;
...
int OpticEngineP4A::GetDutycycleValue ( void* p )
{
  DmdDdp442XInterface::GetDutycycleValue ( reinterpret_cast<uint8_t*> ( p ) );
  return 0;
}

int OpticEngineP4A::SetLedDacRed ( void* p )
{
  /*  red yellow */
...
int OpticEngineP4A::SetLedDacRedBasic ( void* p )
{
  SetLedDacRed ( p );
...
int OpticEngineP4A::GetLedDacRedBasic ( void* p )
{
  dac_t* dac = reinterpret_cast<dac_t*> ( p );
...
int OpticEngineP4A::GetLedDacRed ( void* p )
{
  dac_cfg_t dac_cfg;
...
int OpticEngineP4A::SetLedDacGreen ( void* p )
{
  /*  green yellow cyan */
  dac_t* dac = reinterpret_cast<dac_t*> ( p );
  dac_cfg_t dac_cfg;
...
int OpticEngineP4A::SetLedDacGreenBasic ( void* p )
{
  SetLedDacGreen ( p );
...
int OpticEngineP4A::GetLedDacGreenBasic ( void* p )
{
  dac_t* dac = reinterpret_cast<dac_t*> ( p );
...
int OpticEngineP4A::GetLedDacGreen ( void* p )
{
  dac_cfg_t dac_cfg;
...
int OpticEngineP4A::SetLedDacBlue ( void* p )
{
  /*  blue cyan */
  dac_t* dac = reinterpret_cast<dac_t*> ( p );
  dac_cfg_t dac_cfg;
...
int OpticEngineP4A::SetLedDacBlueBasic ( void* p )
{
  SetLedDacBlue ( p );
  dac_cfg_t* pdac_cfg = reinterpret_cast<dac_cfg_t*> ( p );
...
int OpticEngineP4A::GetLedDacBlueBasic ( void* p )
{
  dac_t* dac = reinterpret_cast<dac_t*> ( p );
...
int OpticEngineP4A::GetLedDacBlue ( void* p )
{
  dac_cfg_t dac_cfg;
...
int OpticEngineP4A::LumCompStart()
{
  return 0;
}

int OpticEngineP4A::SetDspEffect ( uint32_t dspEffect )
{
...
int OpticEngineP4A::GetMaxDac ( void* p )
{
...
int OpticEngineP4A::GetDmdTemp ( int& temp )
{
  int16_t t;
...
int OpticEngineP4A::GetLedTemp ( int* ptemp )
{
...
int OpticEngineP4A::GetLedStatus ( void* p )
{
  DmdDdp442XInterface::GetLedTempVA ( reinterpret_cast<uint8_t*> ( p ) );
  return 0;
}

int OpticEngineP4A::SetDmdRegVal ( uint8_t subaddr, uint32_t datalen, uint8_t* data )
{
  return DmdDdp442XInterface::SetCommand ( subaddr, data, datalen );
}

int OpticEngineP4A::GetDmdRegVal ( uint8_t subaddr, uint32_t paramlen, uint8_t* param, uint32_t datalen, uint8_t* data )
{
  return DmdDdp442XInterface::GetCommand ( subaddr, param, paramlen, data, datalen );
}

int OpticEngineP4A::SetWpcMode ( uint8_t mode )
{
  int imode = mode;
...
int OpticEngineP4A::GetWpcMode ( uint8_t& mode )
{
  int imode = 0;
...
int OpticEngineP4A::GetAlgorithmEnable ( uint8_t& value )
{
...
int OpticEngineP4A::SetCCATarget ( uint8_t* pTarget )
{
...
int OpticEngineP4A::GetCCATarget ( uint8_t* pTarget )
{
  auto iter = map_cca_str.find ( pTarget[0] );
...
int OpticEngineP4A::SetHsgAdjustment ( void* hsg )
{
...
int OpticEngineP4A::GetHsgAdjustment ( void* hsg )
{
  uint8_t* pTemp = reinterpret_cast<uint8_t*> ( hsg );
...
bool OpticEngineP4A::SetFanPwm ( uint8_t fanid, uint8_t pwm ) /*  0:; 1:led driver; 2:led,; 3: */
{
  DmdDdp442XInterface::SetFanPwm ( fanid, pwm );
  return true;
}

bool OpticEngineP4A::GetFanPwm ( uint8_t fanid, uint8_t& pwm )
{
  DmdDdp442XInterface::GetFanPwm ( fanid, pwm );
  return true;
}

bool OpticEngineP4A::SetFanSpeed ( uint8_t fanid, uint16_t speed )
{
  return true;
}

bool OpticEngineP4A::GetFanSpeed ( uint8_t fanid, uint16_t& speed )
{
  int i = 0;
...
uint8_t* OpticEngineP4A::GetEdid ( )
{
  return OpticEngineP4AEDID;
}

int OpticEngineP4A::SetOverLap ( uint16_t overlap )
{
  return 0;
}

int OpticEngineP4A::GetOverLap ( uint16_t& overlap )
{
  return 0;
}

int OpticEngineP4A::GetFirmwareVersion ( uint32_t& version )
{
  uint8_t ver[4] = {0};
  ...
  {
    PRINT_LOG ( LOG_LEVEL_NORMAL, "Get firmware verison error, ret = %d!\n", ret );
  }

  int* pTmp = ( int* ) ver;
  PRINT_LOG ( LOG_LEVEL_NORMAL, "Get firmware verison %d!\n", *pTmp );

  return ret;
}

int OpticEngineP4A::CheckLedTempAndSetFanPwm()
{
  return 0;
  static int i = 0;
  ++i;
  ...
  int temp = 0 ;
  uint8_t mode = 0;
  uint8_t fan1Pwm, fan2Pwm, fan3Pwm;
  uint8_t fan1PwmNow, fan2PwmNow, fan3PwmNow;

  GetDmdTemp ( temp );
  GetBcMode ( mode );

  if ( mode < 6 ) /* mode < 6 means 100%, else 130% */
  {
    if ( temp < 35 )
    {
      fan1Pwm = 80;
  ...
  PRINT_LOG ( LOG_LEVEL_NORMAL, "dmd temperature %d, system mode %d, fan1 pwm %d, fan2 pwm %d, fan3 pwm %d\n",
              temp, mode, fan1Pwm, fan2Pwm, fan3Pwm );

  GetFanPwm ( 1, fan1PwmNow );
  GetFanPwm ( 2, fan2PwmNow );
  GetFanPwm ( 3, fan3PwmNow );
  ...
  if ( fan3PwmNow != fan3Pwm )
  {
    int32_t looptimes = 3;

    while ( ! ( SetFanPwm ( 3, fan3Pwm ) ) && ( looptimes-- ) )
    {
      PRINT_LOG ( LOG_LEVEL_NORMAL, "set fan3 pwm failed %d\n", looptimes );
      usleep ( 500 );
    }
  ...
  PRINT_LOG ( LOG_LEVEL_NORMAL, "actually, fan1 pwm %d, fan2 pwm %d, fan3 pwm %d\n",
