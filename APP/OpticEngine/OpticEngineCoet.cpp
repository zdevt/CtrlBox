...
std::shared_ptr<OpticEngine> OpticEngineCoet::GetObj ( std::string engineName )
{
  return std::make_shared<OpticEngineCoet> ( engineName );
}

OpticEngineCoet::OpticEngineCoet ( std::string engineName )
  : OpticEngine ( engineName )
{
}

OpticEngineCoet::~OpticEngineCoet()
{
}

uint32_t OpticEngineCoet::GetEngineRes ( int& width, int& height )
{
  width = 1920;
  height = 1080;
  return OPTIC_ENGINE_RES_1080P;
}
uint32_t OpticEngineCoet::GetEngineType()
{
  return OPTIC_ENGINE_TYPE_LED;
}

uint32_t OpticEngineCoet::GetEnginePlatform()
{
  return OPTIC_ENGINE_PLATFORM_DDP442X;
}

uint32_t OpticEngineCoet::GetEngine2Dor3D()
{
  return OPTIC_ENGINE_2D;
}

int OpticEngineCoet::WriteLogoAndTestPic()
{
...
int OpticEngineCoet::WriteLogo()
{
  FpgaTestPic::WriteBmpLogo ( "/app/logo_1080P.bmp", FPGA_PIC_SXGA );
  return 0;
}

int OpticEngineCoet::WriteTestPic()
{
  FpgaTestPic::WriteBmpTestPic ( "/app/test_pic_1080P.bmp", FPGA_TEST_PIC );
  return 0;
}

int OpticEngineCoet::SetPowerState ( uint8_t state )
{
  PRINT_LOG ( LOG_LEVEL_NORMAL, "%d\n", state );

  uint8_t poweronoff = state ? 0x00 : 0x07;
  ...
  if ( state )
  {
    ...
    SetFpgaPixel();
    sleep ( 1 );
    DmdDdp442XInterface::SetCommand ( 0x01, &poweronoff, 1 );
    /*  set saved dac */
...
int OpticEngineCoet::GetPowerState ( uint8_t& state )
{
  int istate = 0;
  int temp = 1;
  #if defined(__arm__)
...
int OpticEngineCoet::SetFpgaPixel( )
{
  int w = 0;
  int h = 0;
  GetEngineRes ( w, h );
...
int OpticEngineCoet::SetBcMode ( uint8_t mode )
{
...
int OpticEngineCoet::GetBcMode ( uint8_t& mode )
{
  int imode = 0;
  int temp = 0;
...
int OpticEngineCoet::SetDmdGamma ( uint8_t gammaType )
{
...
int OpticEngineCoet::GetDmdGamma ( uint8_t& gammaType )
{
  int igammaType = 0;
  int temp = 0;
...
int OpticEngineCoet::SetProjMode ( uint8_t mode )
{
...
int OpticEngineCoet::GetProjMode ( uint8_t& mode )
{
  int imode = 0;
  int temp = 0;
...
int OpticEngineCoet::SetColorOffset ( uint16_t r, uint16_t g, uint16_t b )
{
...
int OpticEngineCoet::GetColorOffset ( uint16_t& r, uint16_t& g, uint16_t& b )
{
...
int OpticEngineCoet::SetColorBrightness ( uint16_t r, uint16_t g, uint16_t b )
{
...
int OpticEngineCoet::GetColorBrightness ( uint16_t& r, uint16_t& g, uint16_t& b )
{
...
int OpticEngineCoet::SetColorContrast ( uint16_t r, uint16_t g, uint16_t b )
{
...
int OpticEngineCoet::GetColorContrast ( uint16_t& r, uint16_t& g, uint16_t& b )
{
...
int OpticEngineCoet::SetDutycycle ( uint8_t dc )
{
  int idc = dc;
...
int OpticEngineCoet::GetDutycycle ( uint8_t* dc )
{
  int idc;
  int temp = 0;
...
int OpticEngineCoet::GetDutycycleValue ( void* p )
{
  DmdDdp442XInterface::GetDutycycleValue ( reinterpret_cast<uint8_t*> ( p ) );
  return 0;
}

int OpticEngineCoet::SetLedDacRed ( void* p )
{
  /*  red yellow */
...
int OpticEngineCoet::SetLedDacRedBasic ( void* p )
{
  SetLedDacRed ( p );
...
int OpticEngineCoet::GetLedDacRedBasic ( void* p )
{
  dac_t* dac = reinterpret_cast<dac_t*> ( p );
...
int OpticEngineCoet::GetLedDacRed ( void* p )
{
  dac_cfg_t dac_cfg;
...
int OpticEngineCoet::SetLedDacGreen ( void* p )
{
  /*  green yellow cyan */
  dac_t* dac = reinterpret_cast<dac_t*> ( p );
  dac_cfg_t dac_cfg;
...
int OpticEngineCoet::SetLedDacGreenBasic ( void* p )
{
  SetLedDacGreen ( p );
...
int OpticEngineCoet::GetLedDacGreenBasic ( void* p )
{
  dac_t* dac = reinterpret_cast<dac_t*> ( p );
...
int OpticEngineCoet::GetLedDacGreen ( void* p )
{
  dac_cfg_t dac_cfg;
...
int OpticEngineCoet::SetLedDacBlue ( void* p )
{
  /*  blue cyan */
  dac_t* dac = reinterpret_cast<dac_t*> ( p );
  dac_cfg_t dac_cfg;
...
int OpticEngineCoet::SetLedDacBlueBasic ( void* p )
{
  SetLedDacBlue ( p );
  dac_cfg_t* pdac_cfg = reinterpret_cast<dac_cfg_t*> ( p );
...
int OpticEngineCoet::GetLedDacBlueBasic ( void* p )
{
  dac_t* dac = reinterpret_cast<dac_t*> ( p );
...
int OpticEngineCoet::GetLedDacBlue ( void* p )
{
  dac_cfg_t dac_cfg;
...
int OpticEngineCoet::LumCompStart()
{
  return 0;
}

int OpticEngineCoet::SetDspEffect ( uint32_t dspEffect )
{
...
int OpticEngineCoet::GetMaxDac ( void* p )
{
...
int OpticEngineCoet::GetDmdTemp ( int& temp )
{
  int16_t t;
...
int OpticEngineCoet::GetLedTemp ( int* ptemp )
{
...
int OpticEngineCoet::GetLedStatus ( void* p )
{
  DmdDdp442XInterface::GetLedTempVA ( reinterpret_cast<uint8_t*> ( p ) );
  return 0;
}

int OpticEngineCoet::SetDmdRegVal ( uint8_t subaddr, uint32_t datalen, uint8_t* data )
{
  return DmdDdp442XInterface::SetCommand ( subaddr, data, datalen );
}

int OpticEngineCoet::GetDmdRegVal ( uint8_t subaddr, uint32_t paramlen, uint8_t* param, uint32_t datalen, uint8_t* data )
{
  return DmdDdp442XInterface::GetCommand ( subaddr, param, paramlen, data, datalen );
}

int OpticEngineCoet::SetWpcMode ( uint8_t mode )
{
  int imode = mode;
...
int OpticEngineCoet::GetWpcMode ( uint8_t& mode )
{
  int imode = 0;
...
int OpticEngineCoet::GetAlgorithmEnable ( uint8_t& value )
{
...
int OpticEngineCoet::SetCCATarget ( uint8_t* pTarget )
{
...
int OpticEngineCoet::GetCCATarget ( uint8_t* pTarget )
{
  auto iter = map_cca_str.find ( pTarget[0] );
...
int OpticEngineCoet::SetHsgAdjustment ( void* hsg )
{
...
int OpticEngineCoet::GetHsgAdjustment ( void* hsg )
{
  uint8_t* pTemp = reinterpret_cast<uint8_t*> ( hsg );
...
bool OpticEngineCoet::SetFanPwm ( uint8_t fanid, uint8_t pwm ) /*  0:; 1:led driver; 2:led,; 3: */
{
  DmdDdp442XInterface::SetFanPwm ( fanid, pwm );
  return true;
}

bool OpticEngineCoet::GetFanPwm ( uint8_t fanid, uint8_t& pwm )
{
  DmdDdp442XInterface::GetFanPwm ( fanid, pwm );
  return true;
}

bool OpticEngineCoet::SetFanSpeed ( uint8_t fanid, uint16_t speed )
{
  return true;
}

bool OpticEngineCoet::GetFanSpeed ( uint8_t fanid, uint16_t& speed )
{
  int i = 0;
...
uint8_t* OpticEngineCoet::GetEdid ( )
{
  return OpticEngineCoetEDID;
}

int OpticEngineCoet::SetOverLap ( uint16_t overlap )
{
  return 0;
}

int OpticEngineCoet::GetOverLap ( uint16_t& overlap )
{
  return 0;
}

int OpticEngineCoet::GetFirmwareVersion ( uint32_t& version )
{
  uint8_t ver[4] = {0};
...
int OpticEngineCoet::SetChannelSource ( uint8_t type )
{
  return DmdDdp442XInterface::SetChannelSource ( type );
}

int OpticEngineCoet::GetChannelSource ( uint8_t& type )
{
